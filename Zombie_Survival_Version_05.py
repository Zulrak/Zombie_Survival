#  Source File Name: Zombie_Survival_Version_05
#  Author's Name: Jordan Cooper
#  Last Modified By: Jordan Cooper
#  Date Last Modified: Tuesday, Aug 13, 2013
""" 
  Program Description:  Zombie Survival is about Doug the zombie. Doug, the lovable zombie is back from his
        ...
        Version: 0.5   - Splash Screen added
                       - Updated the Graphics 
                       - Added a new level EXTREEM
...
class DougTheZombie(pygame.sprite.Sprite):
    def __init__(self,spitAcid):
        ...
        if not pygame.mixer:
            ...
            self.backgroundSound = pygame.mixer.Sound("ObstacleCourse.wav")
...
# Create the flying enemy, the Bat          
...
# Move the bat left 7px Until the bat leaves the player's feild of view
    def update(self):
        self.rect.centerx -= self.dx
        if self.rect.right < 0:
            self.reset()
            
# Reset the bat to a randomized position BEHIND the player's feild of view           
    def reset(self):
        self.rect.left = 0
        self.rect.centerx = random.randrange(700, 1000)
        self.rect.centery = random.randrange(250, 280)
...
class Ground(pygame.sprite.Sprite):
    def __init__(self, difficultyLevel):
        ...
        if difficultyLevel == 2:
            self.image = pygame.image.load("Medium.gif")
        elif difficultyLevel == 3:
            self.image = pygame.image.load("Hard.gif")
        elif difficultyLevel == 4:
            self.image = pygame.image.load("HellMode.gif")
        else:
...
def GameScreen(difficultyLevel):
    screen = pygame.display.set_mode((640, 480))
    pygame.display.set_caption("Zombie Survival Version 0.5")
    ...
    if difficultyLevel == 1:
        ...
        enemySprites = pygame.sprite.OrderedUpdates(enemy1,enemy2)
    ...
    # Medium Difficulty. 100% Health
        ...
        enemySprites = pygame.sprite.OrderedUpdates(enemy1,enemy2,enemy3,enemy4)
    # Hard Difficulty. 25% Less health
        ...
        enemySprites = pygame.sprite.OrderedUpdates(enemy1,enemy2,enemy3,enemy4,enemy5)
        ...
        enemySprites = pygame.sprite.OrderedUpdates(enemy1,enemy2,enemy3,enemy4,flyingEnemy1)
    
    friendSprites     = pygame.sprite.OrderedUpdates(ground,dougTheZombie,spitAcid)
    brainSprites      = pygame.sprite.OrderedUpdates(brain1,brain2,brain3)
    ...
    while keepGoing:
        ...
        # Collision detection acid VS enemies (+ 1 Brain)
...
def instructions(brainCount):
    ...
    # if it is the player's first time playing the game display the splash screen
    # else send them to the instructions screen right away
    if brainCount == 0:
        background = pygame.image.load("SplashScreen.gif") 
        screen.blit(background, (0, 0))
    else:
        seconds = 5
